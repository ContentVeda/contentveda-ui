...
export default function TimerWidget(props: TimerWidgetProps) {
  ...
  return (
    <div
      ref={rootRef}
      class={`cv-timer-widget cv-timer-variant-${props.variant || 'dark'} ${state.hasBackgroundImage ? 'cv-timer-has-bg' : ''} ${props.className || ''}`}
      style={{
    ...
    >
      ...
      <div class="cv-timer-content" style={{ position: state.contentOverlaysBox ? 'absolute' : 'relative', top: state.contentOverlaysBox ? 0 : undefined, left: state.contentOverlaysBox ? 0 : undefined, width: state.contentOverlaysBox ? '100%' : undefined, height: state.contentOverlaysBox ? '100%' : undefined }}>
        {props.title && <h3 class="cv-timer-title">{props.title}</h3>}
        <Show when={!state.isExpired}>
          <div class="cv-timer-blocks" role="timer" aria-live="off" aria-label={`Time remaining: ${state.timeLeft.days} days, ${state.timeLeft.hours} hours, ${state.timeLeft.minutes} minutes, ${state.timeLeft.seconds} seconds`}>
            <div class="cv-timer-block">
              <span class="cv-timer-value" aria-hidden="true">{state.timeLeft.days}</span>
              <span class="cv-timer-label" aria-hidden="true">Days</span>
            </div>
            <div class="cv-timer-block">
              <span class="cv-timer-value" aria-hidden="true">{state.timeLeft.hours}</span>
              <span class="cv-timer-label" aria-hidden="true">Hours</span>
            </div>
            <div class="cv-timer-block">
              <span class="cv-timer-value" aria-hidden="true">{state.timeLeft.minutes}</span>
              <span class="cv-timer-label" aria-hidden="true">Minutes</span>
            </div>
            <div class="cv-timer-block">
              <span class="cv-timer-value" aria-hidden="true">{state.timeLeft.seconds}</span>
