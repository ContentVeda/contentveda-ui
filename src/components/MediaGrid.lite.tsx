...
export default function MediaGrid(props: MediaGridProps) {
  ...
  return (
    <div ref={rootRef} class={`cv-media-grid ${props.className || ''}`}>
      ...
      <Show when={!state.showSkeleton}>
        {props.primaryMedia && (
          <a href={props.primaryMedia.mapLinks?.[0]?.url || undefined} class="cv-media-primary" aria-label={props.primaryMedia.altText || props.primaryMedia.title || "Media content"}>
        ...
        {props.secondaryMedia && props.secondaryMedia.length > 0 && (
          <div class="cv-media-secondary-col">
            {props.secondaryMedia.map((item) => (
              <a href={item.mapLinks?.[0]?.url || undefined} class="cv-media-secondary-item" key={item.id} aria-label={item.altText || item.title || "Media content"}>
