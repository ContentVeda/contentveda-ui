...
export default function Banner(props: BannerProps) {
  ...
  return (
    <div
      ref={rootRef}
      class={`cv-banner ${state.showSkeleton ? 'cv-image-shimmer' : ''} ${props.className || ''}`}
      style={{
    ...
    >
      ...
      <Show when={state.shouldMount && !!props.hotspots?.length}>
        <div class="cv-banner-hotspots" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 2 }}>
          ...
          >
            {props.hotspots?.map((h) => (
              ...
              <g class={`cv-hotspot-visual cv-hotspot-visual-${h.shape}`}>
                ...
                {h.pulse && (
                  <Show when={h.shape === 'rect'}>
                    <rect class="cv-hotspot-pulse-ring" x={h.coords.x} y={h.coords.y} width={h.coords.width} height={h.coords.height} vector-effect="non-scaling-stroke" />
                  </Show>
                )}
                {h.pulse && (
                  <Show when={h.shape === 'oval'}>
                ...
                {h.pulse && (
                  <Show when={h.shape === 'polygon'}>
                    <polygon class="cv-hotspot-pulse-ring" points={state.hotspotPolygonPoints(h)} vector-effect="non-scaling-stroke" />
                  </Show>
                )}
              </g>
              </g>
          ...
          {props.hotspots?.map((h) => (
            <div key={h.id}>
              <div class="cv-hotspot-hit" style={state.hotspotHitStyle(h)}>
                <a
                  href={state.hotspotHref(h)}
                  aria-label={state.hotspotLabel(h)}
                  aria-describedby={h.showTooltip ? `cv-hotspot-tip-${h.id}` : undefined}
                  class={`cv-hotspot cv-hotspot-${h.shape}`}
                >
                  <Show when={!!h.showTooltip}>
                    <span id={`cv-hotspot-tip-${h.id}`} role="tooltip" class="cv-hotspot-tooltip">
                      {h.label || h.altText}
                    </span>
                  </Show>
                </a>
      ...
      >
        ...
        >
          <Show when={state.showSkeleton}>
            <div class="cv-skeleton-title cv-image-shimmer" style={{ width: '60%', height: '36px', marginBottom: '16px' }} />
            <div class="cv-skeleton-text cv-image-shimmer" style={{ width: '80%', height: '18px', marginBottom: '10px' }} />
            <div class="cv-skeleton-text cv-image-shimmer" style={{ width: '50%', height: '18px', marginBottom: '24px' }} />
