...
export default function AlternatingSlider(props: AlternatingSliderProps) {
  ...
  return (
    <div
      ref={rootRef}
      class={`cv-alt-slider ${state.showSkeleton ? 'cv-image-shimmer' : ''} ${props.className || ''}`}
                ...
                >
                  <Show when={slideRow[colIndex]}>
                    <Show when={slideRow[colIndex].mapLinks?.[0]?.url}>
                      <a class="cv-alt-content-wrap" href={slideRow[colIndex].mapLinks[0].url} style={{ display: 'block', textDecoration: 'none', color: 'inherit' }}>
                        <Show when={state.shouldMount && slideRow[colIndex].media?.type === 'video'}>
                          <video
                            ...
                            muted
                            playsInline
                            class={`cv-alt-bg-video ${state.showSkeleton ? 'cv-image-shimmer' : ''}`}
                        ...
                        <Show when={state.shouldMount && slideRow[colIndex].media?.type !== 'video'}>
                          <div
                            ...
                            class={`cv-alt-bg ${state.showSkeleton ? 'cv-image-shimmer' : ''}`}
                          />
                        </Show>
                        ...
                        >
                          <Show when={state.showSkeleton}>
                            <div class="cv-skeleton-title cv-image-shimmer" style={{ width: '60%', height: '24px', marginBottom: '12px' }} />
                            <div class="cv-skeleton-text cv-image-shimmer" style={{ width: '80%', height: '14px', marginBottom: '8px' }} />
                            <div class="cv-skeleton-text cv-image-shimmer" style={{ width: '50%', height: '14px', marginBottom: '16px' }} />
                            <div class="cv-skeleton-button cv-image-shimmer" style={{ width: '110px', height: '36px' }} />
                          </Show>
                          <Show when={!state.showSkeleton}>
                            <h2 class="cv-alt-title">{slideRow[colIndex].title}</h2>
                            <Show when={slideRow[colIndex].subtitle}>
                              <p class="cv-alt-subtitle">{slideRow[colIndex].subtitle}</p>
                            </Show>
                            <Show when={slideRow[colIndex].ctaText}>
                              <span class="cv-alt-cta">{slideRow[colIndex].ctaText}</span>
                            </Show>
                          </Show>
                        </div>
                      </a>
