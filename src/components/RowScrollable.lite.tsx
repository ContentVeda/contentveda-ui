...
export default function RowScrollable(props: RowScrollableProps) {
  ...
  return (
    <div ref={containerRef} class={`cv-scrollable-container ${props.className || ''}`}>
      {props.title && <h3 class="cv-scrollable-title">{props.title}</h3>}

      <div class="cv-scrollable-wrapper" style={{ position: 'relative' }}>
        <div
          ref={rowRef}
          class={`cv-scrollable-row ${props.config?.hideScrollbar ? 'cv-scrollable-hide-scrollbar' : ''}`}
        >
                ...
                {item.media?.url && (
                  <div class="cv-scrollable-img-wrap">
                    <Show when={item.media?.type === 'video'}>
                      <video src={item.media?.url} autoPlay loop muted playsInline class="cv-scrollable-img" />
                    </Show>
                    <Show when={item.media?.type !== 'video'}>
                      <img src={item.media?.url} alt={item.title || ''} class="cv-scrollable-img" />
                    </Show>
                  </div>
                )}
                <div class="cv-scrollable-body">
                  {item.title && <div class="cv-scrollable-card-title">{item.title}</div>}
                  {item.subtitle && <div class="cv-scrollable-card-sub">{item.subtitle}</div>}
                </div>
              </Show>
            </a>
          ))}
