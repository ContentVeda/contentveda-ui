Feature: Solid target
  As a consumer of @contentveda/ui
  I want every component's compiled Solid output to render correctly
  So that Solid apps get the same behavior as the Svelte and Web Component targets

  Scenario: Banner renders content and animates its background effect
    Given I mount the "Banner" Solid component with:
  ...
  Scenario: AnnouncementBar renders a linked message
    Given I mount the "AnnouncementBar" Solid component with:
  ...
  Scenario: GridBanner renders every item as a card
    Given I mount the "GridBanner" Solid component with:
      | columns | 3                                                                                                                                                                                                              |
  ...
  Scenario: MediaGrid renders primary and secondary media
    Given I mount the "MediaGrid" Solid component with:
  ...
  Scenario: RowScrollable renders its title and items
    Given I mount the "RowScrollable" Solid component with:
  ...
  Scenario: RichTextEditor renders its toolbar and initial content
    Given I mount the "RichTextEditor" Solid component with:
  ...
  Scenario: Banner renders without errors in SSR mode
    Given I SSR mount the "Banner" "Solid" component with:
      | title | SSR Test |
    Then it should render without any page errors
