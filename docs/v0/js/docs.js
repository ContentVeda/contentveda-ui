/* ============================================================
   ContentVeda UI Docs — JavaScript
...
function generateLiveDemoCode() {
  ...
  const rawCode = codeEl.innerText.replace(
    /(["'])((?:\.\.\/)?assets\/[^"'\s]+)\1/g,
    (match, quote, relPath) => quote + new URL(relPath, 'https://docs.contentveda.com/ui/').href + quote
  );
  ...
  if (framework === 'react') {
    ...
    jsCode = rawCode
      ...
      .replace(/from\s+['"]@contentveda\/ui\/(.*?)['"]/g, `from 'https://esm.sh/@contentveda/ui@latest/$1?deps=react@${REACT_VERSION},react-dom@${REACT_VERSION}'`)
      .replace(/import\s+['"]@contentveda\/ui\/(.*?)['"]/g, `import 'https://esm.sh/@contentveda/ui@latest/$1?deps=react@${REACT_VERSION},react-dom@${REACT_VERSION}'`);
