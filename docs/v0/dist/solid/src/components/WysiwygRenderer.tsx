import { onMount, on, createEffect, createMemo, createSignal } from "solid-js";
...
function WysiwygRenderer(props: WysiwygRendererProps) {
  ...
  const renderedHtml = createMemo(() => {
    return shouldMount() ? props.htmlContent : "";
  });
  ...
  const onUpdateFn_0_props_htmlContent = createMemo(() => props.htmlContent);
  const onUpdateFn_0_props_widgetData = createMemo(() => props.widgetData);
  function onUpdateFn_0() {
    if (shouldMount()) processContent();
  }
  createEffect(
    on(
      () => [onUpdateFn_0_props_htmlContent(), onUpdateFn_0_props_widgetData()],
      onUpdateFn_0
    )
  );
  ...
  return (
    <>
      <div
        class={`cv-wysiwyg-content ${
          !shouldMount() ? "cv-image-shimmer" : ""
        } ${props.className || ""}`}
        ref={containerRef!}
        style={{
          "min-height": !shouldMount() ? "120px" : "",
        }}
