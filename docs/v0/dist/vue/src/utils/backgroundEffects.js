...
function renderWaves(ctx, canvas, ctxBox) {
    let time = 0;
    const animate = () => {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function renderRain(ctx, canvas, ctxBox, stormy) {
...
function renderSun(ctx, canvas, ctxBox, rising) {
...
function drawFrostShard(ctx, x, y, size, rotation, opacity, points) {
    if (opacity <= 0 || size <= 0)
        return;
    ctx.save();
    ctx.translate(x, y);
...
function renderFog(ctx, canvas, ctxBox) {
    const layers = [
    ...
    const frostShards = [];
    for (let i = 0; i < 34; i++)
        frostShards.push(makeFrostShard());
    let lastFrameT = 0;
    const animate = (t) => {
        if (t - lastFrameT < 32) {
            ctxBox.animationFrameId = requestAnimationFrame(animate);
...
function renderAutumn(ctx, canvas, ctxBox) {
    const leaves = [];
    for (let i = 0; i < 42; i++)
        leaves.push(makeLeaf(canvas));
    let lastFrameT = 0;
    const animate = (t) => {
        if (t - lastFrameT < 32) {
            ctxBox.animationFrameId = requestAnimationFrame(animate);
...
function makeFirework(canvas, t) {
...
function renderFestival(ctx, canvas, ctxBox) {
    const sparkles = [];
    for (let i = 0; i < 46; i++)
        sparkles.push(makeSparkle(canvas));
    let fireworks = [];
...
function drawSantaSleigh(ctx, x, y, scale, t, dir = 1) {
    ctx.save();
    ctx.translate(x, y);
...
function drawPeekingCap(ctx, x, y, edgeAngle, reveal, t) {
    if (reveal <= 0.01)
        return;
    ctx.save();
    ctx.translate(x, y);
...
function drawCandyCane(ctx, x, y, size, rotation) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
    ctx.lineCap = 'round';
...
function drawMintCandy(ctx, x, y, size, rotation) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
...
function renderSanta(ctx, canvas, ctxBox) {
    ...
    const snow = [];
    for (let i = 0; i < 40; i++)
        snow.push(makeSnowflake(canvas));
    const candies = [];
    for (let i = 0; i < 10; i++)
        candies.push(makeCandy(canvas));
    ...
    const animate = (t) => {
        ...
        for (let i = 0; i < candies.length; i++) {
            ...
            if (c.kind === 'cane')
                drawCandyCane(ctx, x, c.y, c.size, c.rotation);
            else
                drawMintCandy(ctx, x, c.y, c.size, c.rotation);
            c.y += c.speedY;
        ...
        if (leg === 'out' || leg === 'return') {
            ...
            if (progress > 1) {
                ...
                if (t - lastDropAt > 190 + Math.random() * 150) {
                    ...
                    if (candies.length > 22)
                        candies.splice(0, candies.length - 22);
                }
            }
        }
        ...
        if (cap.phase !== 'hidden') {
            let reveal = 1;
            if (cap.phase === 'in')
                reveal = 1 - Math.max(0, (cap.changeAt - t) / capInMs);
            else if (cap.phase === 'out')
                reveal = Math.max(0, (cap.changeAt - t) / capOutMs);
...
function renderSea(ctx, canvas, ctxBox) {
    const blobs = [];
    for (let i = 0; i < 40; i++)
        blobs.push(makeFoamBlob());
    const filaments = [];
    for (let i = 0; i < 26; i++)
        filaments.push(makeFoamFilament());
    let lastFrameT = 0;
    const animate = (t) => {
        if (t - lastFrameT < 32) {
            ctxBox.animationFrameId = requestAnimationFrame(animate);
