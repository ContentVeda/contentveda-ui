...
function RichTextEditor(props: RichTextEditorProps) {
  ...
  function confirmWidget() {
    ...
    restoreSelection();
    let html = `<div class="cv-widget" data-widget="${selectedWidget}" style="padding: 24px; border: 2px dashed rgba(139,92,246,0.5); background: rgba(139,92,246,0.05); text-align: center; border-radius: 12px; margin: 16px 0; color: #a78bfa; font-weight: 600;">[ContentVeda Widget: ${selectedWidget.toUpperCase()}]</div><p><br></p>`;
    document.execCommand("insertHTML", false, html);
    syncContent();
  }
  ...
  function confirmSocial() {
    ...
    if (socialUrl) {
      restoreSelection();
      let embedHtml = `<div class="social-embed-placeholder" data-platform="${socialPlatform}" data-url="${socialUrl}" style="padding: 24px; border: 2px dashed rgba(14, 165, 233, 0.5); background: rgba(14, 165, 233, 0.05); text-align: center; border-radius: 12px; margin: 16px 0; color: #38bdf8; font-weight: 600;">[Embedded ${socialPlatform.toUpperCase()} Post: ${socialUrl}]</div><p><br></p>`;
    ...
    >
      ...
      >
        ...
        {showToolbarOption("headings") ? (
          <select
            className="bg-black/20 border border-white/10 text-slate-300 font-semibold text-sm rounded-lg px-3 py-1.5 outline-none focus:border-violet-500 transition-colors cursor-pointer"
            value={headingFormat}
            onMouseDown={(e) => {
              saveSelection();
            }}
        ...
        showToolbarOption("social") ? (
          <div className="flex items-center gap-2 text-slate-300">
            ...
            {activeFormats.inTable && showToolbarOption("table") ? (
              <div className="flex items-center bg-violet-500/20 rounded-lg p-0.5 border border-violet-500/30 ml-1 mr-1 shadow-inner">
                <button
                  type="button"
                  className="w-7 h-7 flex items-center justify-center rounded hover:bg-violet-500/40 text-violet-300 transition-colors"
                  title="Add Row Below"
                  onMouseDown={(e) => e.preventDefault()}
                ...
                <div className="w-px h-4 bg-violet-500/30 mx-0.5" />
                <button
                  type="button"
                  className="w-7 h-7 flex items-center justify-center rounded hover:bg-violet-500/40 text-violet-300 transition-colors"
        ...
        {showToolbarOption("insertButton") || showToolbarOption("addWidget") ? (
          <div className="flex items-center gap-2">
            ...
            {showToolbarOption("addWidget") ? (
              <button
                type="button"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-xs transition-all duration-200 bg-pink-500/10 text-pink-400 border-none hover:bg-pink-500/20"
                onMouseDown={(e) => e.preventDefault()}
        ...
        {showToolbarOption("classInput") ? (
          <div className="ml-auto flex items-center bg-black/20 border border-white/10 rounded-lg px-3 py-1.5 shadow-inner focus-within:border-violet-500 focus-within:ring-1 focus-within:ring-violet-500 transition-all">
      ...
      <div
        className="editor-source flex-1 overflow-y-auto bg-[#020617] min-h-[350px]"
        style={{
          display: mode === "source" ? "block" : "none",
        }}
      >
        <textarea
          className="w-full h-full p-6 bg-transparent text-emerald-400 font-mono text-[14px] leading-loose outline-none resize-none"
